...
- **按钮级多模态渲染**：支持按钮声明 `outputFormat`，Dock 自动呈现 **JSON 折叠语法树**、**Markdown 图文** 与 **Mermaid 高清架构图**。
- **AI 自动化接入与跨 Agent 运维**：完备的 RESTful API，人类与 AI 共享一套项目运维资产，换用任何新 Agent 均可一键接管，杜绝脚本散落。
...
| **按钮级多模态输出** | 按钮声明 `outputFormat`，Dock 自动精准直显（📄 文本 / {} JSON 树 / 📋 Markdown 与 **Mermaid 架构图**） |
| **工作区 Master-Detail 底部 Dock** | 底部沉浸式工作区，左栏项目隔离执行记录流水账，右栏多模态输出实时渲染、一键复制与 Grep 过滤 |
| **跨 Agent 运维中枢** | 集中收敛管理各项目常用运维命令，杜绝不同 Agent 把脚本乱写在各目录 |
...
```bash
npm test            # 运行全部 54 项自动化测试用例 (node --test)
```
...
- [x] **按钮级输出格式声明与 Dock 自动多模态渲染**：支持按按钮定义 `outputFormat`（ANSI 彩色文本 / JSON 折叠语法树 / Markdown 与 Mermaid 架构图），Dock 自动精准直显。
- [x] **跨 Agent 协同运维中枢**：统一项目运维资产，消除跨工具脚本散落，提供标准 4 步接管 SOP。
